...
mod tests {
    ...
    #[test]
    fn erased_roundtrip() {
        let (type_id, format_id) = make_binary_format("query-binary-test-1");
        test_slice_reader(format_id, type_id);
    }
    ...
    fn idempotent_message_id() {
        #[query]
        struct ReRegisterQuery;
        let first = try_register_query::<ReRegisterQuery>().unwrap();
        let second = try_register_query::<ReRegisterQuery>().unwrap();
        assert_eq!(first, second);
        assert_eq!(second, ReRegisterQuery::type_message_id().unwrap())
    }
            ...
            .map(|_| {
                ...
                thread::spawn(move || {
                    ...
                    for round in 0..ROUNDS {
                        let from_register = try_register_query::<TestQuery>()
                            .unwrap_or_else(|e| panic!("round {round}: register failed: {e}"));
                        let from_lookup = TestQuery::type_message_id()
