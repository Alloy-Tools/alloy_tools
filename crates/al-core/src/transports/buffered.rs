...
impl<T: TransportItemRequirements> std::fmt::Debug for Buffered<T>  {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("Buffered").field(&self.0.producer()).field(&self.1).finish()
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{Buffered, Command, Queue, Transport};
    ...
    #[tokio::test]
    async fn send_recv() {
        let buffered = Buffered::new(Queue::<Command>::new().into());
        buffered.send(Command::Stop).await.unwrap();
        buffered.send_batch(vec![Command::Stop, Command::Stop]).await.unwrap();
        // Recv `String` asynchronously
        assert_eq!(buffered.recv().await.unwrap(), Command::Stop);
        // Try recv `String` asynchronously
        assert_eq!(buffered.try_recv().await.unwrap().unwrap(), Command::Stop);
        // Recv avaliable `String` asynchronously
        assert_eq!(buffered.recv_avaliable().await.unwrap(), vec![Command::Stop]);

        buffered.send_blocking(Command::Stop).unwrap();
        buffered.send_batch_blocking(vec![Command::Stop, Command::Stop]).unwrap();
        tokio::time::sleep(std::time::Duration::from_nanos(1)).await;
        // Recv `String` synchronously
        assert_eq!(buffered.recv_blocking().unwrap(), Command::Stop);
        // Try recv `String` synchronously
        assert_eq!(buffered.try_recv_blocking().unwrap().unwrap(), Command::Stop);
        // Recv avaliable `String` synchronously
        assert_eq!(buffered.recv_avaliable_blocking().unwrap(), vec![Command::Stop]);
    }
}