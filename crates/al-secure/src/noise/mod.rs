...
mod tests {
    use crate::noise::{
        handshake_pattern::HandshakePattern, handshake_state::HandshakeResult, KeyPair, Noise,
        PublicKey, KEY_SIZE, MAX_MSG_BYTE_LEN,
    };
    ...
    #[test]
    fn nn_handshake() {
        let mut initiator = Noise::new(HandshakePattern::NN)
            .initiate::<RandomTimeStamp<Microseconds>>()
            .unwrap();

        let mut responder = Noise::new(HandshakePattern::NN)
